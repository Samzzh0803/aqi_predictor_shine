...
**2. Architecture (60s).** Open `ARCHITECTURE.md`'s diagram or `docs/REPORT.md` §4. Walk the arrow: Open-Meteo → feature engineering (one function, every pipeline) → feature store → four models → Model Registry → API → dashboard. Both the Feature Store and the Model Registry are genuinely live on Hopsworks — say so plainly. Automation and public hosting are the two pieces still deferred; name them too.
...
**4. Feature Store (30s).** Open the live Feature Store in the Hopsworks UI — `aqi_features_v1`, `aqi_targets_v1`, and the `aqi_fv_v1` Feature View, 35,712 / 35,545 rows. "This is real and live, not a local stand-in — a fresh process can rebuild the full training set from here alone."
...
**7. Automation / CI (45s).** Open the GitHub Actions tab, show the green CI run. "This is real and running on every push. Hourly and daily retraining automation is scoped but not built yet — it's no longer infrastructure-blocked, since both Hopsworks stores are live now; it's simply the next increment."
...
- **Hopsworks unreachable during the demo →** fall back to `data/metrics/day5_summary.json` and the SHAP PNGs already on disk; the Model Registry and Feature Store screenshots from `docs/REPORT.md` §13-14 stand on their own.
